...
export default function MuiImageList() {
    ...
    //used to display collection of images in organised grid. Example : building receipe website.
    return (
        <Stack spacing={4}>
            {/* Standard imageList */}
            ...
            {/* variant masonry is used here. */}
            <Box sx={{ width: 500, height: 450,overflowY:'scroll' }}>
            <ImageList variant='masonry'  cols={3} gap={8}>
                {itemData2.map((item) => (
                    <ImageListItem key={item.img}>
                        <img src={`${item.img}?w=164&fit=crop&auto=format&dpr=2`}
                            alt={item.title}
                            loading='lazy' />
